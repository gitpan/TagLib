...
=head1 SYNOPSIS
  ...
  my $i = TagLib::MPEG::Header->new($data);
  print $i->layer(), "\n"; # normally got 3
...
The MPEG Version. C<keys %TagLib::MPEG::Header::_Version> lists all
...
There are a few combinations or one or two channel audio that are
 possible. C<keys %TagLib::MPEG::Header::_ChannelMode> lists all
 available values used in Perl code.
