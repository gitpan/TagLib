...
can_ok("TagLib::MPC::File", @methods) 							or 
	diag("can_ok failed");

my $file = "sample/Discontent.mp3";
my $i = TagLib::MPC::File->new($file);
isa_ok($i, "TagLib::MPC::File") 								or 
	diag("method new(file) failed");
isa_ok($i->tag(), "TagLib::Tag") 								or 
	diag("method tag() failed");
isa_ok($i->audioProperties(), "TagLib::MPC::Properties") 		or 
