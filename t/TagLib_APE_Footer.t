...
BEGIN { use_ok('TagLib::APE::Footer') };
...
can_ok("TagLib::APE::Footer", @methods) 			or 
	diag("can_ok failed");

my $i = TagLib::APE::Footer->new();
cmp_ok($i->version(), "==", 0)					or 
	diag("new() failed");
cmp_ok(TagLib::APE::Footer->new(TagLib::ByteVector->new("blah"))->version(), 
...
$i->setData(TagLib::ByteVector->new("blah"x8));
...
cmp_ok(TagLib::APE::Footer->size(), "==", 32)	or 
	diag("method size() failed");
like($i->fileIdentifier()->data(), qr(^APETAGEX)) 		
	or diag("method fileIdentifier() failed");
like(TagLib::APE::Footer->fileIdentifier()->data(), qr(^APETAGEX)) 
	or diag("method fileIdentifier() failed");
