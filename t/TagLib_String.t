...
can_ok("TagLib::String", @methods) or diag("can_ok failed");

my $i = TagLib::String->new();
my $s_latin1 = TagLib::String->new(TagLib::String->new("blah blah"));
is($s_latin1->to8Bit(), "blah blah")						or
	diag("method new(ascii) failed");
is(TagLib::String->new(TagLib::ByteVector->new(
...
cmp_ok($s_latin1->find(TagLib::String->new("ah")), "==", 2) 	or 
	diag("method find(string) failed");
cmp_ok($s_latin1->find(TagLib::String->new("ah"), 4), "==", 7) 	or 
...
is($s_utf16be->substr(0, 2)->to8Bit("true"), $utf8_hardcode) 	or
	diag("method substr(position, n) failed");
is($s_latin1->append(TagLib::String->new(" blah"))->to8Bit(), 
...
cmp_ok($s_utf8->size(), "==", length($s_utf8->to8Bit("true"))) 	or
	diag("method size() failed");
ok(TagLib::String->new()->isEmpty()) 							or
	diag("method isEmtpy() failed");
ok(not $s_latin1->isEmpty()) 									or
	diag("method isEmtpy() failed");
ok(TagLib::String->null()->isNull()) 							or 
...
cmp_ok(TagLib::String->new("a")->toInt(), "==", oct("a")) 		or
	diag("method toInt() failed");
is(TagLib::String->new("   blah   ")->stripWhiteSpace()->to8Bit(), 
...
is($s_utf8->getChar(1), "\x{7684}") 							or 
	diag("method getChar(i) failed");
ok($s_latin1 == TagLib::String->new("blah blah")) 				or 
	diag("method _equal(s, '') failed");
ok($s_utf8 == TagLib::String->new($utf8_hardcode)) 				or 
	diag("method _equal(s, '') failed");
$s_latin1 += " blah";
is($s_latin1->toCString(), "blah blah blah") 					or 
	diag("method _append(string) failed");
$s_latin1 += TagLib::String->new(" blah");
is($s_latin1->toCString(), "blah blah blah blah") 				or 
	diag("method _append(String) failed");
$s_utf8 += "test";
is($s_utf8->toCString("true"), $utf8_hardcode . "test") 		or 
	diag("method _append(string) failed");
ok(TagLib::String->new("a") < TagLib::String->new("b")) 		or 
	diag("method _lessThan(string) failed");
ok(TagLib::String->new("b") > TagLib::String->new("a")) 		or 
	diag("method _lessThan(string) failed");
is(TagLib::String->number(10)->toCString(), "10") 				or 
