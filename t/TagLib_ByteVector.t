...
BEGIN { use_ok('TagLib::ByteVector') };
...
can_ok('TagLib::ByteVector', @methods) or diag("can_ok failed");

my $i = TagLib::ByteVector->new();
is($i->data(), undef)						or 
	diag("new an empty object failed");
is(TagLib::ByteVector->new(3, "a")->data(), "aaa")		or 
	diag("new an object with size and padding char failed");
cmp_ok(TagLib::ByteVector->new(3)->size(), '==', 3)		or 
	diag("new an object with size and char=0 failed");
$i->setData("blah blah blah");
is(TagLib::ByteVector->new($i)->data(), "blah blah blah") 	or 
	diag("copy constructor failed");
################################################################
# is(TagLib::ByteVector->new("t")->data(), "t");
# WILL FAILED
# the data will be printed as "t "
# and length(TagLib::ByteVector->new("t")->data()) got 4
# This might be a bug of Test::More since
# <code>TagLib::ByteVector->new("t")->data() eq "t";</code>
# in a normal script will get true
################################################################
cmp_ok(TagLib::ByteVector->new("t")->size(), "==", 1)		or 
	diag("new an object with char failed");
is(TagLib::ByteVector->new("blah", length("blah"))->data(), "blah")		or 
	diag("new an object with string and length failed");
cmp_ok(TagLib::ByteVector->new("blah", 10)->size(), "==", 10)	or 
	diag("new an object with string and length failed");
is(TagLib::ByteVector->new("blah")->data(), "blah") 		or 
	diag("new an object with string, check data failed");
cmp_ok(TagLib::ByteVector->new("blah")->size(), "==", 4) 	or 
	diag("new an object with string, check size failed");

TODO: {
local $TODO = "bug of size";
my $i = TagLib::ByteVector::->new("blah blah blah");
$i->setData("BLAH");
...
my $i = TagLib::ByteVector::->new("blah blah");
...
is($i->at(4), " ")			or 
	diag("method at failed");
{
my $j = TagLib::ByteVector->new("blah");
...
ok($i->endsWithPartialMatch(TagLib::ByteVector->new("a blah"))) or 
	diag("method endsWithPartialMatch failed");
}
$i->append(TagLib::ByteVector->new(" blah"));
is($i->data(), "blah blah blah blah")	or 
	diag("method append failed");

TODO: {
local $TODO = "bug of clear";
my $i = TagLib::ByteVector->new("blah");
$i->clear();
is($i->data(), undef) 			or 
...
my $ibegin = $i->begin();
isa_ok($ibegin, "TagLib::ByteVector::Iterator") 	or 
	diag("method begin() failed");
my $iend = $i->end();
isa_ok($iend, "TagLib::ByteVector::Iterator") 		or 
	diag("method end() failed");

ok(not $i->isNull()) 			or 
	diag("method isNull failed");
ok(TagLib::ByteVector->null()->isNull()) or 
	diag("method isNull failed");
ok(not $i->isEmpty())			or 
	diag("method isEmpty failed");
ok(TagLib::ByteVector->null()->isEmpty()) or
	diag("method isEmpty failed");
...
$i->setItem(0, "b");
ok($i == TagLib::ByteVector->new($i))	or 
	diag("operator== failed");
ok($i != TagLib::ByteVector->new())	or 
	diag("operator!= failed");
ok($i > TagLib::ByteVector->new("a"))	or 
	diag("operator> failed");
ok($i < TagLib::ByteVector->new("f"x20)) or 
	diag("operator< failed");
{
my $j = TagLib::ByteVector->new("bl") + 
	TagLib::ByteVector->new("ah");
is($j->data(), "blah")			or 
	diag("operator+ failed");
}
{
...
my $k = TagLib::ByteVector->new("bl");
my $j = $k;
$j += TagLib::ByteVector->new("ah");
is($j->data(), "blah")			or 
	diag("operator= failed");
}
{
my $x = TagLib::ByteVector->new("blah");
cmp_ok(TagLib::ByteVector->fromUInt($x->toUInt())->toUInt(), 
	"==", $x->toUInt()) or
	diag("static method fromUInt failed");
}
{
my $y = TagLib::ByteVector->new("bl");
cmp_ok(TagLib::ByteVector->fromShort($y->toShort())->toShort(), 
	"==", $y->toShort()) or 
...
my $z1 = TagLib::ByteVector->new("blahblah");
my $z2 = TagLib::ByteVector->new("blahblah");
is(TagLib::ByteVector->fromLongLong($z1->toLongLong())->toLongLong(), 
	TagLib::ByteVector->fromLongLong($z2->toLongLong())->toLongLong()) or 
...
#is(TagLib::ByteVector->fromCString("blah")->data(), "blah") or 
