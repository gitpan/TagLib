...
can_ok("TagLib::MPEG::File", @methods) 							or 
	diag("can_ok failed");

my $file = "sample/Discontent.mp3";
my $i = TagLib::MPEG::File->new($file);
isa_ok($i, "TagLib::MPEG::File") 								or 
	diag("method new(file) failed");
isa_ok($i->tag(), "TagLib::Tag") 								or 
	diag("method tag() failed");
isa_ok($i->audioProperties(), "TagLib::MPEG::Properties") 		or 
...
isa_ok($i->APETag(1), "TagLib::APE::Tag") 						or 
