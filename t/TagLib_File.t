...
clear tell length );
can_ok("TagLib::File", @methods) 								or 
	diag("can_ok failed");

my $file = "sample/Discontent.mp3";
my $fileref = TagLib::FileRef->new($file);
my $i = $fileref->file();
is($i->name(), $file) 											or 
...
cmp_ok($i->find(TagLib::ByteVector->new("4")), "==", 1255) 		or 
	diag("method find(pattern) failed");
$i->seek(0, "End");
cmp_ok($i->tell(), "==", $i->length()) 							or 
	diag("method seek() and length() failed");
cmp_ok($i->rfind(TagLib::ByteVector->new("4"), 20), "==", -1) 	or 
...
ok(TagLib::File->isReadable(__FILE__))							or 
	diag("method isReadable(file) failed");
ok(TagLib::File->isWritable(__FILE__)) 							or 
