...
BEGIN { use_ok('TagLib::ID3v2::AttachedPictureFrame') };
...
can_ok("TagLib::ID3v2::AttachedPictureFrame", @methods) 			or 
	diag("can_ok failed");

my $i = TagLib::ID3v2::AttachedPictureFrame->new();
isa_ok($i, "TagLib::ID3v2::AttachedPictureFrame") 					or 
	diag("method new() failed");
$i->setTextEncoding("UTF8");
isa_ok(TagLib::ID3v2::AttachedPictureFrame->new($i->render()), 
	"TagLib::ID3v2::AttachedPictureFrame") 							or 
	diag("method new(data) failed");

is($i->textEncoding(), "UTF8") 										or 
	diag("method setTextEncoding(encode) and textEncoding() failed");
$i->setMimeType(TagLib::String->new("image/jpeg"));
...
$i->setPicture(TagLib::ByteVector->new("a dummy picture"));
