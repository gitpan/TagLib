...
BEGIN { use_ok('TagLib::ID3v1::GenreMap') };
...
can_ok("TagLib::ID3v1::GenreMap", @methods) 					or 
	diag("can_ok failed");

my $i = TagLib::ID3v1::GenreMap->new();
isa_ok($i, "TagLib::ID3v1::GenreMap") 							or 
	diag("method new() failed");
isa_ok(TagLib::ID3v1::GenreMap->new($i), 
	"TagLib::ID3v1::GenreMap") 									or 
...
isa_ok($i->begin(), "TagLib::ID3v1::GenreMap::Iterator") 		or 
	diag("method begin() failed");
isa_ok($i->end(), "TagLib::ID3v1::GenreMap::Iterator") 			or 
	diag("method end() failed");
my $key = TagLib::String->new("key");
my $item = 1;
isa_ok($i->find($key), "TagLib::ID3v1::GenreMap::Iterator") 	or 
...
################################################################
tie my %j, "TagLib::ID3v1::GenreMap", $i;
isa_ok(tied %j, "TagLib::ID3v1::GenreMap") 						or 
