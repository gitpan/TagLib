...
can_ok("TagLib::FLAC::File", @methods) 							or 
	diag("can_ok failed");

my $file = "sample/Discontent.flac";
my $i = TagLib::FLAC::File->new($file);
isa_ok($i, "TagLib::FLAC::File") 								or 
	diag("method new(file) failed");
isa_ok($i->tag(), "TagLib::Tag") 								or 
	diag("method tag() failed");
isa_ok($i->audioProperties(), "TagLib::FLAC::Properties") 		or 
	diag("method audioProperties() failed");
isa_ok($i->ID3v2Tag(1), "TagLib::ID3v2::Tag") 					or 
	diag("method ID3v2Tag(t) failed");
isa_ok($i->ID3v1Tag(1), "TagLib::ID3v1::Tag") 					or 
	diag("method ID3v1Tag(t) failed");
isa_ok($i->xiphComment(1), "TagLib::Ogg::XiphComment") 			or 
	diag("method xiphComment(t) failed");
$i->setID3v2FrameFactory(TagLib::ID3v2::FrameFactory->instance());
isa_ok($i->streamInfoData(), "TagLib::ByteVector") 				or 
