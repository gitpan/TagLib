...
CODE:
	...
	case 3:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::StringList")) 
	...
	case 2:
		if(sv_isobject(ST(1))) {
			if(sv_derived_from(ST(1), "TagLib::StringList")) {
				l = INT2PTR(TagLib::StringList *, SvIV(SvRV(ST(1))));
				RETVAL = new TagLib::StringList(*l);
			} else if(sv_derived_from(ST(1), "TagLib::String")) {
...
CODE:
	if(items == 2) {
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::String"))
...
PPCODE:
	 ...
	 */
	if(sv_isobject(ST(1))) {
		if(sv_derived_from(ST(1), "TagLib::String")) {
			s = INT2PTR(TagLib::String *, SvIV(SvRV(ST(1))));
			(void)THIS->append(*s);
		} else if(sv_derived_from(ST(1), "TagLib::StringList")) {
			l = INT2PTR(TagLib::StringList *, SvIV(SvRV(ST(1))));
			(void)THIS->append(*l);
		} else
