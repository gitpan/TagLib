...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::ID3v2::FrameList"))
			l = INT2PTR(TagLib::ID3v2::FrameList *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type TagLib::ID3v2::FrameList");
...
CODE:
	if(sv_isobject(ST(1))) {
		if(sv_derived_from(ST(1), "TagLib::ID3v2::Frame")) {
			item = INT2PTR(TagLib::ID3v2::Frame *, SvIV(SvRV(ST(1))));
			RETVAL = new TagLib::ID3v2::FrameList(THIS->append(item));
		} else if(sv_derived_from(ST(1), "TagLib::ID3v2::FrameList")) {
			l = INT2PTR(TagLib::ID3v2::FrameList *, SvIV(SvRV(ST(1))));
			RETVAL = new TagLib::ID3v2::FrameList(THIS->append(*l));
		} else
			croak("ST(1) is not of type TagLib::ID3v2::Frame/TagLib::ID3v2::FrameList");
	} else
...
PPCODE:
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::ID3v2::Frame", (void *)item);
...
PPCODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			...
			if(SvREADONLY(SvRV(ST(1)))){
				...
				ST(0) = sv_newmortal();
				sv_setref_pv(ST(0), "TagLib::ID3v2::FrameList", (void *)
	...
	default:
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::FrameList", (void *)list);
	}
	XSRETURN(1);
...
PPCODE:
	if(items > 1) {
		...
		for(int i = 1; i < items; i++) {
			if(!(sv_isobject(ST(i)) && sv_derived_from(ST(i), "TagLib::ID3v2::Frame") || 
				sv_derived_from(ST(i), "TagLib::ID3v2::FrameList")))
				croak("ST(i) is not of type TagLib::ID3v2::Frame/TagLib::ID3v2::FrameList");
		}
		for(int i = 1; i < items; i++) {
			if(sv_derived_from(ST(i), "TagLib::ID3v2::Frame"))
...
PPCODE:
	if(!THIS->isEmpty()) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::Frame", (void *)THIS->back());
		SvREADONLY_on(SvRV(ST(0)));
		it = THIS->end();
		THIS->erase(--it);
		XSRETURN(1);
...
PPCODE:
	if(!THIS->isEmpty()) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::Frame", (void *)THIS->front());
		SvREADONLY_on(SvRV(ST(0)));
...
PPCODE:
	...
	default:
		...
		for(int i = 3; i < items; i++) {
			if(!(sv_isobject(ST(i)) && 
				sv_derived_from(ST(i), "TagLib::ID3v2::Frame") || 
				sv_derived_from(ST(i), "TagLib::ID3v2::FrameList")))
			croak("ST(i) is not of type TagLib::ID3v2::Frame/TagLib::ID3v2::FrameList");
		}
		it = THIS->begin();
	...
	if(length > 0) {
		...
		for(int i = 0; i < length; i++) {
			item = (*THIS)[offset];
			ST(i) = sv_newmortal();
			sv_setref_pv(ST(i), "TagLib::ID3v2::Frame", (void *)item);
