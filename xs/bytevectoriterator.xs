...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::ByteVector::Iterator"))
			i = INT2PTR(TagLib::ByteVector::Iterator *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type TagLib::ByteVector::Iterator");
...
PPCODE:
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::ByteVector::Iterator", (void *)&i);
...
PPCODE:
	...
	//ST(0) = sv_newmortal();
	//sv_setref_pv(ST(0), "TagLib::ByteVector::Iterator", (void *)THIS);
	//SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
