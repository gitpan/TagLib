...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1))) {
			if(sv_derived_from(ST(1), "TagLib::File")) {
			...
			} else if(sv_derived_from(ST(1), "TagLib::FileRef")) {
...
PPCODE:
	...
	if(t != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::Tag", (void*)t);
...
PPCODE:
	...
	if(p != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::AudioProperties", (void*)p);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
...
PPCODE:
	...
	if(f != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::File", (void*)f);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
...
PPCODE:
	...
	if(file != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::File", (void *)file);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
