...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::Ogg::FieldListMap::Iterator"))
			i = INT2PTR(TagLib::Ogg::FieldListMap::Iterator *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type TagLib::Ogg::FieldListMap::Iterator");
...
//USEPAIR
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::StringList", (void *)&data);
...
PPCODE:
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::Ogg::FieldListMap::Iterator", (void *)&i);
	SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
...
PPCODE:
	...
	//ST(0) = sv_newmortal();
	//sv_setref_pv(ST(0), "TagLib::Ogg::FieldListMap::Iterator", (void *)THIS);
	//SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
