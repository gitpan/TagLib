...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "_NAMESPACE_"))
			l = INT2PTR(_NAMESPACE_ *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type _NAMESPACE_");
...
CODE:
	if(sv_isobject(ST(1))) {
		if(sv_derived_from(ST(1), "_T_")) {
			item = INT2PTR(_T_ *, SvIV(SvRV(ST(1))));
			RETVAL = new _NAMESPACE_(THIS->append(item));
		} else if(sv_derived_from(ST(1), "_NAMESPACE_")) {
			l = INT2PTR(_NAMESPACE_ *, SvIV(SvRV(ST(1))));
			RETVAL = new _NAMESPACE_(THIS->append(*l));
		} else
			croak("ST(1) is not of type _T_/_NAMESPACE_");
	} else
...
PPCODE:
	_T_ * item = THIS->front();
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "_T_", (void *)item);
...
PPCODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			...
			if(SvREADONLY(SvRV(ST(1)))){
				...
				ST(0) = sv_newmortal();
				sv_setref_pv(ST(0), "_NAMESPACE_", (void *)
	...
	default:
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "_NAMESPACE_", (void *)list);
	}
	XSRETURN(1);
...
PPCODE:
	if(items > 1) {
		...
		for(int i = 1; i < items; i++) {
			if(!(sv_isobject(ST(i)) && sv_derived_from(ST(i), "_T_") || 
				sv_derived_from(ST(i), "_NAMESPACE_")))
				croak("ST(i) is not of type _T_/_NAMESPACE_");
		}
		for(int i = 1; i < items; i++) {
			if(sv_derived_from(ST(i), "_T_"))
...
PPCODE:
	if(!THIS->isEmpty()) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "_T_", (void *)THIS->back());
		SvREADONLY_on(SvRV(ST(0)));
		it = THIS->end();
		THIS->erase(--it);
		XSRETURN(1);
...
PPCODE:
	if(!THIS->isEmpty()) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "_T_", (void *)THIS->front());
		SvREADONLY_on(SvRV(ST(0)));
...
PPCODE:
	...
	if(length > 0) {
		...
		for(int i = 0; i < length; i++) {
			item = (*THIS)[offset];
			ST(i) = sv_newmortal();
			sv_setref_pv(ST(i), "_T_", (void *)item);
