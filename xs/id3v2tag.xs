...
PPCODE:
	h = THIS->header();
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::ID3v2::Header", (void *)h);
...
PPCODE:
	...
	if(eh != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::ExtendedHeader", 
			(void *)eh);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
...
PPCODE:
	...
	if(f != NULL) {
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::Footer", (void *)f);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
...
PPCODE:
	...
	default:
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::FrameList", (void *)&list2);
	}
	SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
