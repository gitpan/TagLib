...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::Ogg::FieldListMap"))
			m = INT2PTR(TagLib::Ogg::FieldListMap *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type TagLib::Ogg::FieldListMap");
...
PPCODE:
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::StringList", (void *)&item);
...
PPCODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			...
			if(SvREADONLY(SvRV(ST(1)))){
				ST(0) = sv_newmortal();
				sv_setref_pv(ST(0), "TagLib::Ogg::FieldListMap", (void *)
	...
	default:
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::Ogg::FieldListMap", (void *)map);
	}
	XSRETURN(1);
...
PPCODE:
	...
	if(THIS->contains(*key)) {
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::StringList", (void *)&value);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
...
PPCODE:
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::String", (void *)&key);
	SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
