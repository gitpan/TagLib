...
CODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			sv_derived_from(ST(1), "TagLib::ID3v2::FrameListMap"))
			m = INT2PTR(TagLib::ID3v2::FrameListMap *, SvIV(SvRV(ST(1))));
		else
			croak("ST(1) is not of type TagLib::ID3v2::FrameListMap");
...
PPCODE:
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::ID3v2::FrameList", (void *)&item);
...
PPCODE:
	...
	case 2:
		if(sv_isobject(ST(1)) && 
			...
			if(SvREADONLY(SvRV(ST(1)))){
				ST(0) = sv_newmortal();
				sv_setref_pv(ST(0), "TagLib::ID3v2::FrameListMap", (void *)
	...
	default:
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::FrameListMap", (void *)map);
	}
	XSRETURN(1);
...
PPCODE:
	...
	if(THIS->contains(*key)) {
		...
		ST(0) = sv_newmortal();
		sv_setref_pv(ST(0), "TagLib::ID3v2::FrameList", (void *)&value);
		SvREADONLY_on(SvRV(ST(0)));
		XSRETURN(1);
	} else
		XSRETURN_UNDEF;
...
PPCODE:
	...
	ST(0) = sv_newmortal();
	sv_setref_pv(ST(0), "TagLib::ByteVector", (void *)&key);
	SvREADONLY_on(SvRV(ST(0)));
	XSRETURN(1);
